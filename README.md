...
1.  **Frontend:** Collects user input and the selected "mode", then sends a POST request to the backend.
2.  **Backend:** Receives the request, prepends the appropriate system prompt based on the chosen mode, and forwards the entire conversation history to the Gemini API.
3.  **LLM Provider:** Generates the response and sends it back through the FastAPI server to be displayed on the React frontend.
